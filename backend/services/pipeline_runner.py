...
from src.features import compute_veg_indices, compute_veg_index_features, VEG_INDEX_FORMULAS, VEG_INDEX_CHANNELS
...
def process_plant_image(bucket, key):
    ...
    for _, pdata in flats.items():
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            ...
            for idx, func in VEG_INDEX_FORMULAS.items():
                bands = VEG_INDEX_CHANNELS.get(idx)
                if bands is None:
                    print(f"[WARN] No bands for index {idx}, skipping.")
                    continue
                try:
                    vals = [spec[b].squeeze(-1) for b in bands]
                except KeyError as e:
                    print(f"[WARN] Missing band {e} for index {idx}, skipping.")
                    continue
                futures.append(executor.submit(save_index, idx, func, spec, mask, bucket, prefix))
            for f in futures:
                f.result()
    ...
    def analyze_and_save_texture(pdata, plant_id, bucket, prefix):
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        texture_futures = []
        for _, pdata in flats.items():
            texture_futures.append(executor.submit(analyze_and_save_texture, pdata, plant_id, bucket, prefix))
        for f in texture_futures:
            f.result()
